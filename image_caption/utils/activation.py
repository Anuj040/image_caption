...
def multi_head_attention_forward(
    ...
    attn_output, attn_output_weights = _scaled_dot_product_attention(
        q, k, v, attn_mask, dropout_p, softmax_dim=softmax_dim
    )
...
if __name__ == "__main__":
    layer = CustomMultiheadAttention(
        embed_dim=10,
        num_heads=2,
        dropout_p=0.1,
        bias=True,
    ...
    q = torch.rand(2, 8, 10)
    k = torch.rand(2, 1, 10)
    b = torch.randint(0, 10, (4, 8, 1))
    MASK = (b > 4 - 1).to(float)
    o = layer(query=q, key=k, value=k, need_weights=False, attn_mask=MASK)
